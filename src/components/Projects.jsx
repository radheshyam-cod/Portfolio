              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', transformStyle: 'preserve-3d' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', transform: 'translateZ(10px)' }}>
                    {project.github && (
                      <a href={project.github} target="_blank" rel="noreferrer" aria-label={`View source code for ${project.title}`} title={`View source code for ${project.title}`} style={{ color: 'var(--color-text-muted)', transition: 'color 0.2s' }} onClick={(e) => e.stopPropagation()}>
                        <Github size={18} />
                      </a>
                    )}
                    {project.live && (
